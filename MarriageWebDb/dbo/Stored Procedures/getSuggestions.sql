...
SET @addressCountry = (SELECT a.AddressCountry from UserProfile u join ADDRESS a on a.UserProfileId=u.UserProfileId where u.UserProfileId = @user_profile)
 ...
 If @genderId = 1
	If @orientationId = 1 
		...
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and u.GenderId=2 and u.OrientationId!=2
		END
	Else If @orientationId = 2
		--gay male matches gay/bi/other male
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and u.GenderId=1 and u.OrientationId!=1
		END
	Else If @orientationId = 3
		--bi male matches gay/bi/other male and straight/bi/other female
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and ((u.GenderId=1 and u.OrientationId!=1) or (u.GenderId=2 and u.OrientationId!=2))
		END
	Else
		--other male matches anything?
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile
		END
Else
	If @orientationId = 1 
		--straight female matches straight/bi/other male
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and u.GenderId=1 and u.OrientationId!=2
		END
	Else If @orientationId = 2
		--gay female matches gay/bi/other female
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and u.GenderId=2 and u.OrientationId!=1
		END
	Else If @orientationId = 3
		--bi female matches gay/bi/other female or straight/bi/other male
		BEGIN
			Select UserProfileId from UserProfile u WHERE u.UserProfileId!=@user_profile and ((u.GenderId=1 and u.OrientationId!=2) or (u.GenderId=2 and u.OrientationId!=1))
		END
	Else
